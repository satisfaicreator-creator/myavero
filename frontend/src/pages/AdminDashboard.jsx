...
import { LogOut, Inbox, RefreshCw, Phone, MessageCircle, Mail, Settings, ListChecks } from "lucide-react";
...
const TOP_TABS = [
  { key: "enquiries", label: "Enquiries", icon: ListChecks },
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h1 className="font-heading text-2xl sm:text-3xl font-bold">
              {section === "enquiries" ? "Enquiry Dashboard" : "Site Settings"}
            </h1>
            <p className="text-white/60 text-sm">
              {section === "enquiries"
                ? "Every lead. Every channel. One place."
                : "Update pricing, offer content and the announcement bar live."}
            </p>
          </div>
          ...
          <div className="flex items-center gap-1 glass rounded-full p-1">
            {TOP_TABS.map((t) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs inline-flex items-center gap-2 whitespace-nowrap transition-colors ${
                  section === t.key
                    ? "bg-gradient-to-r from-cyan-500 to-fuchsia-500 text-white"
                    : "text-white/70 hover:text-white"
                }`}
                data-testid={`section-tab-${t.key}`}
              >
